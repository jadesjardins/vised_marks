...
results=inputgui( ...
    ...
    {'Style','popup','string',action_cell,'value',action_val, ...
    ...
    'end; clear tmpcolor;'] } ...
    ...
    {'Style','text','string','New line mark color ([R G B])','enable',line_color_enable}, ...
    ...
    'end; clear tmpcolor;'] } ...
    ...
    {'Style','text','string','New tag mark color ([R G B])','enable',tag_color_enable}, ...
    ...
    'end; clear tmpcolor;'] } ...
    ...
    {'Style','text','string','Mark order','enable',order_enable}, ...
...
switch results{1}
    case 1
        info_type='time_info';
        marks_prop{1}=results{2};
        marks_prop{2}=str2num(results{3});
        marks_prop{3}=zeros(1,EEG.pnts);
    case 2
        info_type='chan_info';
        marks_prop{1}=results{2};
        marks_prop{2}=str2num(results{4});
        marks_prop{3}=str2num(results{5});
        marks_prop{4}=str2num(results{6});
...
for i=1:length(flaglabel);
    clear labelind;
    eval(['labelind=find(strcmp(''',flaglabel{i},''',{EEG.marks.',infotype,'.label}))']);
    if ~isempty(labelind);
        disp(['Clearing ''',flaglabel{i},''' flag type...']);
        eval(['EEG.marks.',infotype,'(',num2str(labelind),')=[];']);
    end
end
